...
import WorkerDatabase, { TodoList, Todo } from './worker-database';
...
function App() {
  ...
  const [workerDb] = useState(() => new WorkerDatabase());
  ...
  const initializeApp = async () => {
    try {
      ...
      workerDb.onDataChange(loadAllData);
      
      const result = await workerDb.initialize();
  ...
  const loadAllData = () => {
    try {
      const todoLists = workerDb.getTodoLists();
      const todos = workerDb.getTodos();
      const changes = workerDb.getChanges();
  ...
  const handleAddTodo = async () => {
    ...
    try {
      await workerDb.addTodo(appState.selectedList, appState.newTodoText.trim());
  ...
  const handleAddList = async () => {
    ...
    try {
      await workerDb.addTodoList(appState.newListName.trim());
      setAppState(prev => ({
        ...prev,
        newListName: '',
  ...
  const handleDeleteList = async (name: string) => {
    try {
      await workerDb.deleteTodoList(name);
  ...
  if (dbStatus === 'initializing') {
    return (
      <div className="app">
        <div className="loading">Initializing shared worker database...</div>
      </div>
    );
  }
  ...
  if (dbStatus === 'error') {
    return (
      <div className="app">
        <h1>CR-SQLite Shared Worker Test</h1>
        <div className="error">
    ...
    <div className="app">
      ...
      <div className="sync-controls">
        ...
        <div className="status" style={{ marginLeft: '20px' }}>
          <strong>Status:</strong> Connected to Shared Worker | 
