...
function App() {
  ...
  const { data: todoLists = [], isLoading: listsLoading } = useTodoLists(db);
  const { data: allTodos = [], isLoading: todosLoading } = useAllTodos(db);
  const { data: changes = [], isLoading: changesLoading } = useChanges(db);
  const { data: selectedTodos = [], isLoading: selectedTodosLoading } = useTodosByList(db, appState.selectedList);

  // Mutations
  const addTodoMutation = useAddTodo(db);
  const toggleTodoMutation = useToggleTodo(db);
  const deleteTodoMutation = useDeleteTodo(db);
  const addTodoListMutation = useAddTodoList(db);
  const deleteTodoListMutation = useDeleteTodoList(db);
